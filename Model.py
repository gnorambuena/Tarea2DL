...
def buildConv2DLayer(input, out_size):
	conv = tf.layers.conv2d(input,out_size,3
			padding = "SAME", activacion = tf.nn.relu,
...
x_norm = tf.layers.batch_normalization(conv,training = True)
...
conv4_1 = buildConv2DLayer(maxpool3,128)
conv4_2 = buildConv2DLayer(conv4_1,128)
